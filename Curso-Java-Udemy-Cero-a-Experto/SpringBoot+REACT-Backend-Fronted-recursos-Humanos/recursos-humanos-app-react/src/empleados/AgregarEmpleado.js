...
export default function AgregarEmpleado() {
  ...
  const onInputChange = (e)=>{
    // spread operator ... (expandir los atributos del objeto del tipo empleado)
    setEmpleado({...empleado, [e.target.name]: e.target.value})
  }

  const onSubmit = async (e)=> {
    e.preventDefault(); // ← ¡EVITA QUE EL FORMULARIO RECARGUE LA PÁGINA PERDIENDO LOS DATOS SI LO HACE!
    const urlBase = "http://localhost:8080/rh-app/empleados";
    await axios.post(urlBase, empleado); // ← ENVÍA LOS DATOS AL BACKEND
    // redirigimos a la pagina de inicio
    navegacion("/"); // previamente ya declarada esta variable --> let navegacion = useNavigate();
  }
